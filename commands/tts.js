...
module.exports = {
  ...
  func: async (sock, msg, text) => {
    ...
    if (!ttsText) return await msg.reply('*Please enter or reply to any text to speak!*');
    ...
    if (ttsMessage.length > 200) return await msg.reply('*❌ Lengthy text, The text you entered is so lengthy to speak, maximum 200 characters only!*');
    let audio = false;
    try {
     audio = await gtts.getAudioUrl(ttsMessage, { lang: lang, slow: isSlow, host: 'https://translate.google.com' });
    } catch {
     return await msg.reply('*❌ Invalid Language, Please enter a valid language code!*');
    }
